...
| TC12 | `movePiece`, `getBoard`, `getCurrentTurn`, `isKingInCheck` | White en passant capture would expose white king to rook check along the rank/file | Move rejected; both pawns remain in place; `isKingInCheck(WHITE)` remains false; turn remains WHITE | ✅ |
| TC13 | `movePiece`, `getBoard`, `getCurrentTurn` | White pawn makes a normal diagonal capture into an occupied square after previous move was a two-square pawn move elsewhere | Normal capture succeeds; en passant state does not incorrectly affect regular capture | ✅ |
