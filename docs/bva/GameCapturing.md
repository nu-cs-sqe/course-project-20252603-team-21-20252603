...
| TC2 | `movePiece`, `getBoard`, `getCurrentTurn` | WHITE piece captures BLACK piece with valid movement pattern: white knight `(7,1)` to `(5,2)`, then black pawn `(1,3)` to `(2,3)`, then white knight `(5,2)` to `(4,4)`, then black pawn `(2,3)` to `(3,3)`, then white knight `(4,4)` to `(2,3)` | BLACK pawn is removed, WHITE knight occupies destination, start square is empty, turn switches to BLACK | :x: |`(3,0)`, then white knight `(3,1)` to `(1,0)` | BLACK pawn is removed, WHITE knight occupies destination, start square is empty, turn switches to BLACK | :x: |
