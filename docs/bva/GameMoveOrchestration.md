...
|---|---|---|---|---|
| TC1 | `movePiece`, `getBoard`, `getCurrentTurn` | Valid WHITE pawn move from `(6,0)` to `(5,0)` | Pawn moves to end square, start square becomes empty, turn switches to BLACK | :x: |
| TC2 | `movePiece` | Start square is empty, moving from `(4,0)` to `(3,0)` | Move is rejected with exception | :x: |
